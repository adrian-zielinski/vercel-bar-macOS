...
14. **AppModel po recenzji Taska 10**: sort migawki ze stabilnym tie-breakiem po nazwie; odczyt pęku w `refresh()` przez `Task.detached` (bez blokowania MainActora), 3 kolejne porażki odczytu → `.tokenInvalid`; offline zeruje `anyActive`, 401 nabija `consecutiveFailures` (backoff), `connect()` zeruje oba liczniki. Świadome odroczenia po v1: sygnalizacja trwałej porażki migawki przy `phase == .normal` (dane zamrożone przy starym „odświeżono HH:MM"), testy maszyny faz AppModel (app target poza zasięgiem runnera).
